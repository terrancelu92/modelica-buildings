...
model HeatPumpPlant_example_template_signal
  extends Modelica.Icons.Example;
  Buildings.Controls.OBC.CDL.Integers.Sources.Constant conInt(k=0)
    annotation (Placement(transformation(extent={{-80,-10},{-60,10}})));
  HHW_CHW_plant_template_signal hHW_CHW_plant_template_signal(datAll(pla(
        ...
        pumHeaWatSec(m_flow_nominal=fill(1.4, hHW_CHW_plant_template_signal.pumHeaWatSec.nPum),
            dp_nominal=fill(120000, hHW_CHW_plant_template_signal.pumHeaWatSec.nPum)),
        hp(
        ...
        pumChiWatSec(dp_nominal(each displayUnit="Pa") = fill(10000,
            hHW_CHW_plant_template_signal.pumChiWatSec.nPum)))), ctl(
      triHeaWat=-0.03,
      kCtlDpHeaWat=0.1,
      TiCtlDpHeaWat=60,
...
equation
  connect(conInt.y, hHW_CHW_plant_template_signal.nReqResChiWat) annotation (
      Line(points={{-58,0},{-14,0},{-14,14},{-4,14}}, color={255,127,0}));
  connect(conInt.y, hHW_CHW_plant_template_signal.nReqPlaChiWat) annotation (
      Line(points={{-58,0},{-14,0},{-14,6},{-4,6}}, color={255,127,0}));
  connect(conInt1.y, hHW_CHW_plant_template_signal.nReqPlaHeaWat) annotation (
      Line(points={{-18,-20},{-10,-20},{-10,0},{-4,0}}, color={255,127,0}));
  connect(ram.y, reaToInt.u)
    annotation (Line(points={{-58,70},{-42,70}}, color={0,0,127}));
  connect(reaToInt.y, hHW_CHW_plant_template_signal.nReqResHeaWat) annotation (
      Line(points={{-18,70},{-14,70},{-14,20},{-4,20}}, color={255,127,0}));
  connect(weather.weaBus, hHW_CHW_plant_template_signal.weaBus) annotation (
      Line(
      points={{-60,-50},{-46,-50},{-46,8},{-22,8},{-22,27.5},{-30.1,27.5}},
      color={255,204,51},
      thickness=0.5));
  connect(bou3.ports[1], hHW_CHW_plant_template_signal.port_a1) annotation (
      Line(points={{30,60},{30,64},{0,64},{0,16}}, color={0,127,255}));
  connect(bou2.ports[1], hHW_CHW_plant_template_signal.port_b1) annotation (
      Line(points={{56,60},{56,64},{70,64},{70,16},{20,16}}, color={0,127,255}));
  connect(hHW_CHW_plant_template_signal.port_b2, coo.port_a)
    annotation (Line(points={{0,4},{0,-30},{20,-30}}, color={0,127,255}));
  connect(coo.port_b, hHW_CHW_plant_template_signal.port_a2) annotation (Line(
        points={{40,-30},{48,-30},{48,4},{20,4}}, color={0,127,255}));
...
end HeatPumpPlant_example_template_signal;
